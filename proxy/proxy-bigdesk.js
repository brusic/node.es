...
var postRequest = function(request, response, responseData) {
    var retval = responseData;

    if (responseData.indexOf("jsonp") == 0) {
        var re = /(^jsonp[0-9]*)\((.*)/;
        var jsonp = re.exec(responseData)[1];
        json = JSON.parse(responseData.replace(/^jsonp[0-9]*\(/,"").replace(/\);$/, ""));
        delete json.indices;
        retval = jsonp + "(" + JSON.stringify(json) + ");";
    } else {
